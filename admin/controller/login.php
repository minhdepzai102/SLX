...
if (isset($_SESSION['role'])) {
  // Nếu session 'role' đã được đặt, chuyển hướng người dùng đến trang tương ứng
  if ($_SESSION['role'] == 1) {
    header('Location: index.php'); // Chuyển hướng đến trang admin nếu vai trò là 1
    exit;
  } else {
    header('Location: ../../user/controller/index.php'); // Chuyển hướng đến trang người dùng nếu vai trò là 0
    exit;
  }
}
    ...
    <div class="form-wrapper sign-in">
      <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post" autocomplete="off">
        ...
        <div class="input-group">
          <input type="text" name="user" required placeholder=" " autocomplete="off" />
          <label for="user">Username</label>
        </div>
        <div class="input-group">
          <input type="password" name="pass" required placeholder=" " autocomplete="new-password" />
